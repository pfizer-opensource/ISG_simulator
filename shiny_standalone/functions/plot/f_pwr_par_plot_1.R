f_pwr_par_plot_1 = function(delta_v , sd = sd ,n_per_arm_fix=n_per_arm_fix, n_trt = n_trt){
  
  pwr_v  = rep(NA, length(delta_v))
  
  for(i in 1: length(delta_v)){
    pwr_v[i] = pwr_hc_par(n = n_per_arm_fix, delta = delta_v[i], sd = sd, K = n_trt, alpha = 0.05, alternative = "two.sided") 
