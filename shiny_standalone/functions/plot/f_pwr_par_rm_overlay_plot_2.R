f_pwr_par_rm_overlay_plot_2 = function(n_v = 2:10, sd_bl = sd_bl, sd_no_bl = sd_no_bl, power_fix = power_fix, n_trt = n_trt) {

  delta_bl_v    = rep(NA, length(n_v))
  delta_no_bl_v = rep(NA, length(n_v))

  for (i in 1:length(n_v)) {
    delta_bl_v[i] = delta_hc_par_rm(
      n = n_v[i],
      sd = sd_bl,
      K = n_trt,
      p = p,
    ...
    delta_no_bl_v[i] = delta_hc_par_rm(
      n = n_v[i],
      sd = sd_no_bl,
      K = n_trt,
      p = p,
  ...
  plot = ggplot(df, aes(y = delta, x = n, color = group, label = label)) +
    ...
    labs(
      ...
      y = "Effect Size (Delta)",
      title = "Parallel-RM: Sample Size vs Effect Size",
...
f_pwr_par_rm_overlay_points_tb_2 = function(sd_bl, sd_no_bl, power_fix, n_trt, p, n_v = 2:10) {
  delta_bl_v    = rep(NA, length(n_v))
  delta_no_bl_v = rep(NA, length(n_v))
  for (i in seq_along(n_v)) {
    delta_bl_v[i]    = delta_hc_par_rm(n = n_v[i], sd = sd_bl,    K = n_trt, p = p, alpha = 0.05, power = power_fix, alternative = "two.sided")
    delta_no_bl_v[i] = delta_hc_par_rm(n = n_v[i], sd = sd_no_bl, K = n_trt, p = p, alpha = 0.05, power = power_fix, alternative = "two.sided")
  }
