f_pwr_xov_curve_data_1 = function(delta_v, sd, n_per_trt_fix, n_trt){
...
f_pwr_xov_points_tb_1 = function(sd, n_per_trt_fix, n_trt, x_points = c(5, 10, 15, 20)) {
  df_points <- f_pwr_xov_curve_data_1(
    delta_v = x_points,
    sd = sd,
    n_per_trt_fix = n_per_trt_fix,
    n_trt = n_trt
  )
...
f_pwr_xov_plot_1 = function(delta_v, sd = sd, n_per_trt_fix = n_per_trt_fix, n_trt = n_trt){

  df <- f_pwr_xov_curve_data_1(
    delta_v = delta_v,
    sd = sd,
    n_per_trt_fix = n_per_trt_fix,
    n_trt = n_trt
  ...
  plot = ggplot(df, aes(y = pwr, x = delta, label = label)) +
    ...
    labs(x = "Effect Size (Delta)",
         y = "Power",
         title = "Crossover: Power vs Effect Size",
          subtitle = paste0("Sample Size per Treatment: ", n_per_trt_fix)) +
