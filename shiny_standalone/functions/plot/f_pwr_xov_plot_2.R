f_pwr_xov_plot_2 = function(n_v = 2:10, sd = sd, power_fix = power_fix, n_trt = n_trt){

  delta_v = rep(NA, length(n_v))

  for(i in 1:length(n_v)){
    delta_v[i] = delta_hc_xov(
      n = n_v[i],
      sd = sd,
      K = n_trt,
      alpha = 0.05,
  ...
  plot = ggplot(df, aes(y = delta, x = n, label = label)) +
    ...
    labs(x = "Sample Size per Treatment (n)",
         y = "Effect Size (Delta)",
         title = "Crossover: Sample Size vs Effect Size",
...
f_pwr_xov_points_tb_2 = function(sd, power_fix, n_trt, n_v = 2:10) {
  delta_v = rep(NA, length(n_v))
  for (i in seq_along(n_v)) {
    delta_v[i] = delta_hc_xov(n = n_v[i], sd = sd, K = n_trt, alpha = 0.05, power = power_fix, alternative = "two.sided")
  }
