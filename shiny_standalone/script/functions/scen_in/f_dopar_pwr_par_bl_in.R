  ...
  M = foreach (i         =  1:n_sim,
               ...
               .packages =  c("dplyr","emmeans","lme4")  ) %dopar% { 
                 ...
                 design_par      <- cbind.data.frame(animal_code     = sample(1:n_ani_pool, n_ani_par,replace = F),
                                                     period_code     = 1,
                                                     treatment_code  = rep(c(1,2,3,4), each = n_per_grp_par),
                 ...
                 mod             <- lm(y ~ baseline + treatment_code , data = design_par)
                 rg              <- ref_grid(mod)
                 ...
                 v_bl            = var(design_par$baseline)
                 smr             = summary(mod);smr
                 rmse            = sqrt(mean(smr$residuals^2))
                 
                 anova           = anova(mod)
                 AMSE            = anova$`Mean Sq`[1] 
                 RMSE            = anova$`Mean Sq`[3]
                 v_bt            = v_bl  #(AMSE-RMSE)/4  
                 v_wt            = RMSE # rmse
